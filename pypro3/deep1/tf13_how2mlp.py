...
print(x_test[:1])
print(scaler.inverse_transform(x_test[:1])) # 표준화 원복해서 보기
...
net1 = Dense(units=30, activation='relu')(input_) # 은닉층
...
model2 = Model(inputs=[input_], outputs=[output]) # 최종적으로 입력과 출력을 지정하여 케라스 모델완성
...
# 5개의 특성(0 ~ 4)은 짧으 경로로, 6개의 특성(2 ~ 7)은  깊은 경로로 보낸다고 가정
input_a = Input(shape=[5], name = 'wide_input') # 층이 복잡할때는 name을 주자
