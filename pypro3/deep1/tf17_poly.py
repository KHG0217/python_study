...
a /= sum([(x - x_var) **2 for x in population_inc])
b = y_var - a * x_var
print('a: ',a, ', b: ', b)
...
for i in range(1,1001):
    optimizer.minimize(compute_cost, var_list=[a,b]) # 잔차제곱의 평균을 최소화
    if i % 100 == 0:
        print(i, 'a: ',a.numpy(), ', b:',b.numpy(), ', cost: ',compute_cost().numpy())
...
print('-'*50)
# 3) 케라스로 다항 회귀선 구하기, 2차함수 회귀선 y = ax2 + bx + c -----------   
...
optimizer = tf.keras.optimizers.Adam(learning_rate=0.07)
for i in range(1,1001):
    optimizer.minimize(compute_cost2, var_list=[a,b,c]) # 잔차제곱의 평균을 최소화
    if i % 100 == 0:
        print(i, 'a: ',a.numpy(), ', b:',b.numpy(), ', c:',c.numpy(), ', cost: ',compute_cost2().numpy())
