# 단순선형회귀 분석 모델(정량적(Regression) 모델)을 작성 (feature : 연속형, label : 연속형) - Keras 사용
# 모델 학습방법) keras의 내장 API를 사용
...
model.compile(optimizer=sgd, loss='mse',metrics=['mse']) # mse -> 분류가 아닌 예측모델이기때문에
lossmetrics = model.evaluate(x_data,y_data) # 학습전 모델 성능 확인
