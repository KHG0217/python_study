...
# label : one-hot : sklearn - OneHotEncoder, keras - to_categorical, numpy - np.eye(), pandas - pd.get_dummies()
print(y.shape)  # (150,)
...
print('--- k-fold(교차 검증) 수행 - 3개 모델 전체를 대상으로 진행---------')
from keras.wrappers.scikit_learn import KerasClassifier
from sklearn.model_selection import cross_val_score
create_model = create_custom_model_func(N_FEATURES, N_CLASSES, 10, 2)
...
# ModelCheckpoint, EarlyStooping 적용 지금은 생략
model.fit(x_train, y_train, epochs=50, batch_size=10, verbose=2)

print('evaluate : ', model.evaluate(x_test,y_test))
pred = np.argmax(model.predict(x_test),axis = 1)
print('예측값  : ', pred)
real_y=np.argmax(y_test, axis=1).reshape(-1,1) # 원래형태로
...
print('예측 결과 : ', np.argmax(new_pred, axis = 1).reshape(-1,1).flatten())
