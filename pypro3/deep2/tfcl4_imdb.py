...
from skimage.measure.tests import test_label
...
print(train_data) # 모든 단어에 대해 인덱싱(고유번호)을 해서 단어 사전을 만듦
print(train_label)
...
# print(reverse_word_index) # {34701: 'fawn', 52006: 'tsukino', 52007:
print(reverse_word_index.get(train_data[0][0]))
print()
...
y_train = train_label # train_label.astype('float32') 해도 됨
y_test =test_label
print()
...
model.add(layers.Dropout(rate=0.3)) # 과적합 방지를 위해 일부 유닛의 일부만 동장하고 일부는 동작하지 않도록 하는 방법이다.
...
print(model.evaluate(x_test,y_test, batch_size=512, verbose=0))
