...
model.add(BatchNormalization()) # 배치 정규화 : 역전파 도중 발생하는 기울기 소실, 폭주 문제등을 해결
model.add(Dense(16, activation='relu'))
model.add(BatchNormalization()) # 배치 정규화 : 역전파 도중 발생하는 기울기 소실, 폭주 문제등을 해결
model.add(Dense(8, activation='relu'))
model.add(BatchNormalization()) # 배치 정규화 : 역전파 도중 발생하는 기울기 소실, 폭주 문제등을 해결
model.add(Dense(1, activation='sigmoid')) # sigmoid 이항분류기 떄문에 마지막 sigmoid
...
print('훈련 전 모델 정확도 : {:5.2f}%'.format(loss))
...
# chkpoint = ModelCheckpoint(filepath=MODEL_DIR + modelPath, monitor = 'loss',mode='auto', 
#                            verbos=0, save_best_only=True)

chkpoint = ModelCheckpoint(filepath=MODEL_DIR + "abc.hdf5", monitor = 'loss',mode='auto', 
                           verbos=0, save_best_only=True) # 계속 덮어쓰기헤서 최종값만 남김
...
print('훈련 후 모델 정확도 : {:5.2f}%'.format(loss))
...
plt.plot(epoch_len, vloss, c='red', label="val_accuracy")
