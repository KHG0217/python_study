# 공공 데이터 (data.go.kr)에서 다운로드한 파일 사용
# 서울 유료 관광지에 외국인(미국,일본,중국) 방문 데이터로 상관분석 시ㅣ
...
def chulbalFunc():
    ...
    jsonTp = json.loads(open(fname, 'r', encoding='utf-8').read()) #String ->dict로 : json decoding
    tour_table = pd.DataFrame(jsonTp, columns=('yyyymm','resNm','ForNum')) # 년월, 관광지명, 입장객 수
    tour_table = tour_table.set_index('yyyymm')
    print(tour_table)
    
    resNm = tour_table.resNm.unique()
    print('관광지명 종류: ',resNm[:5]) # 관광지명 종류:  ['창덕궁' '운현궁' '경복궁' '창경궁' '종묘'] 앞에서부터 5군데
    print()
