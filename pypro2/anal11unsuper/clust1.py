...
print('dist_vec :',dist_vec) # 거리를 계산한 결과값 

row_dist = pd.DataFrame(squareform(dist_vec), columns=labels, index = labels)
print(row_dist) # 표로 정리해서 보기

from scipy.cluster.hierarchy import linkage # 응집형 계층적 군집화 가능
row_clusters = linkage(dist_vec, method="average") 
# method = "average": 거리의 평균이용
...
# linkage의 결과를 계통도(dendrowgram)으로 시각화
