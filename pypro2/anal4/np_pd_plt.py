...
pop_seoul = pd.read_excel('https://github.com/pykwon/python/blob/master/testdata_utf8/population_seoul.xls?raw=true',
                          header=2, usecols='B,D,G,J,N') # 경로에 ?raw=true # 엑슬은 A열 ~
...
data_result = pd.merge(cctv_data, pop_seoul, on='구별')
print(data_result.head(3))
print()

data_result.set_index('구별',inplace=True) # 구별을 인덱스로 바꾸기.
print(data_result.head(3))

# 시각화
plt.figure()
data_result["소계"].plot(kind='barh', grid=True, figsize=(8,8))
plt.title('구별 cctv 설치현황')
plt.show()
print()
