...
# 귀무가설: 성별은 콜레스트롤 양에 차이가 없다.
# 대립가설: 성별은 콜레스트롤 양에 차이가 있다.
#
...
# 남자: 3.220000000000001
# 여자: 3.273333333333334
print()

print(stats.shapiro(man).pvalue,' ' ,stats.shapiro(woman).pvalue)
# 남자 p -value : 0.0376255065202713  정규성 X
# 여자 p -value : 0.0014988253824412823 정규성 X
print()

#정규성 만족인 경우
print(stats.ttest_ind(man,woman))
# Ttest_indResult(statistic=-0.06435846357954361, pvalue=0.949142089962745)
# 해석 : pvalue=0.949142089962745 > 0.05 이므로 귀무가설 채택 대립가설 기각
# 귀무가설: 성별은 콜레스트롤 양에 차이가 없다.
...
# 해석 : pvalue=0.97796630859375 > 0.05 이므로 귀무가설 채택 대립가설 기각
# 귀무가설: 성별은 콜레스트롤 양에 차이가 없다.
print()
...
# 귀무가설: 매년 학기 내 치뤄지는 시험성적의 결과에 따른 학업능력에 변화가 없다.
# 대립가설: 매년 학기 내 치뤄지는 시험성적의 결과에 따른 학업능력에 변화가 있다.

mid = [80, 75, 85, 50, 60, 75, 45, 70, 90, 95, 85, 80]
fin = [90, 70, 90, 65, 80, 85, 65, 75, 80, 90, 95, 95]
print(np.mean(np.array(mid))) # 74.16666666666667
print(np.mean(np.array(fin))) # 81.66666666666667
print(stats.ttest_rel(mid,fin))
# Ttest_relResult(statistic=-2.6281127723493993, pvalue=0.023486192540203194)
# 해석 : pvalue = 0.0234 < 0.05 귀무가설 기각 대립가설 채택
#       대립가설: 매년 학기 내 치뤄지는 시험성적의 결과에 따른 학업능력에 변화가 있다.
