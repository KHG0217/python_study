...
# hp가 mpg에 영향을 준다. 가정하고 선형회귀모델을 작성
# sklearn 제공 모델의 경우 독립변수(x, feature)는 2차원, 종속변수(y, label)는 1차원 배열
# 형식을 갖는다.

x = mtcars[['hp']].values
print(x[:3], x.shape) # hp 열 자료를 2차원 메트릭스로 생성

y = mtcars['mpg'].values
print(y[:3], y.shape) # mpg 열 자료를 1차원 배열 형식으로 추출
...
# y = -0.06822828 * x + 30.098860539622496
...
print('RMSE(평균제곱근오차):' , mean_squared_error(y, pred)) # (실제값, 예측값)
# RMSE(평균제곱근오차): 13.989822298268805
print('결정계수(설명력, r2_score) : ',r2_score(y,pred))
# 결정계수(설명력, r2_score :  0.602437341423934 약 60% 설명하고있다.
print()
