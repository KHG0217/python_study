# 특성공학 중 feature extraction (차원축소 : PCA - 주성분분석)
# 주성분 분석(主成分分析, Principal component analysis; PCA)은 
# 고차원의 데이터를 저차원의 데이터로 환원시키는 기법을 말한다. 
# 이 때 서로 연관 가능성이 있는 고차원 공간의 표본들을 
# 선형 연관성이 없는 저차원 공간(주성분)의 표본으로 변환하기 위해 직교 변환을 사용한다.
# PCA는 선형대수 관점으로 볼 때 입력 데이터의 공분산 행렬을 고유값 분해하고
# 이렇게 구한 고유벡터에 입력 데이터를 선형변환 하는 것이다

# 참고 : PCA 진행 순서
# 1) 입력 데이터세트의 공분산 행렬을 생성한다.
# 2) 공분산 행렬의 고유벡터(행렬의 방향으 그대로, 크기만 변하는 벡터)와 고유값을 계산한다.
# 3) 고유값이 큰 순으로 k(변환 차수)개 만큼 고유벡터를 추출한다.
# 4) 추출된 고유벡터를 이용하여 새롭게 입력 데이터를 변환한다.

# iris dataset 중 sepal(꽃받침) 열의 너비와 길이, 두개를 하나의 열로 차원 축소하기
...
x_low = pca1.fit_transform(x) # 비지도 학습으로 target(y값)은 지정하지 않음
print('x_low : ',x_low,' ', x_low.shape)

x2 = pca1.inverse_transform(x_low) # 원복
print('원복 : ', x2,' ', x2.shape)

print(x_low[0])
print(x2[0,:]) # 원래 값 : 5.1 3.5
...
plt.plot(x2[:, 0], x2[:, 1], 'o-', color='g', markersize=10) # 근사 행렬값(원복값) 표시
plt.xlabel('길이')
plt.ylabel('너비')
plt.title('아이리스 꽃받침 자료')
plt.axis('equal')
