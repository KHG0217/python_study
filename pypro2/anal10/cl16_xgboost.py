...
# 캐글의 산탄데르 고객 만족 데이터 세트에 대해 고객 만족 여부를 
# XGBoost 와 LightGBM 으로 예측해보자.
...
cust_df.drop('ID', axis=1, inplace=True) # ID열 제거
...
xgb_clf.fit(X_train, y_train, 
            early_stopping_rounds=3, # 조기중단 
            eval_metric='auc', # 어큐러시값을 체크 늘어나다가 안늘어날때 3회 해도 안늘면 중단
            eval_set=[(X_train, y_train), (X_test, y_test)]) # 학습도중에 안에있는 데이터로 평가함
...
print('분류 정확도 : ', acc) # 분류 정확도 :  0.9581031307550645